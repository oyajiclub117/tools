...
Private Sub proc_workbook( _
	...
	For Each objWks In p_objWkb.Worksheets
		If InStr(objWks.Name, "ïœçXóöó") _
		Or InStr(objWks.Name, "index") _
		Or InStr(objWks.Name, "LIST") Then
		'-- Nothing
		Else
		'	nothing
'--		If InStr(objWks.Name, "ëóã‡éËí†") _
'--		Or InStr(objWks.Name,"ñ¢çﬁî≠í ímÇÕÇ™Ç´ÅQé¿ñ±é“å§èCópÅQì`ëóÉfÅ[É^") _
'--		Or InStr(objWks.Name,"éQâ¡èÿÅQé¿ñ±é“å§èCópÅQì`ëóÉfÅ[É^") _
'--		Or InStr(objWks.Name,"éQâ¡é“ñºïÎÅQé¿ñ±é“å§èCópÅQì`ëóÉfÅ[É^") _
'--		Or InStr(objWks.Name,"éQâ¡èÿÅQé¿ñ±é“å§èCçƒëóópÅQì`ëóÉfÅ[É^") Then
